...
for i in np.arange(num_nets):
    net = tf.keras.layers.Dense(units=(num_nets - i) * 180, activation='tanh', name='dense_{}'.format(i))(net)

net = tf.keras.layers.Dense(units=180, activation='relu', name='dense_out')(net)
out = tf.keras.layers.Reshape((30, 6), name='output')(net)
